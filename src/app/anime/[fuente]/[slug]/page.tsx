...
import { CODE_TO_FUENTE, type FuenteCode } from '@/lib/slug-utils';

function AnimeContent() {
  const params = useParams();
  const router = useRouter();
  const fuenteCode = params.fuente as string;
  const slug = params.slug as string;

  const fuente = CODE_TO_FUENTE[fuenteCode as FuenteCode] || null;
  ...
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      try {
        const response = await fetch('/api/anime/details', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ slug, fuente }),
        });
  ...
  const handleSelectEpisode = async (episode: Episode) => {
    ...
    try {
      const epNum = parseInt(episode.numero.match(/\d+/)?.[0] || '1');
      const response = await fetch('/api/anime/videos', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ slug, fuente, episodio: epNum }),
      });
      const data = await response.json();
          ...
          <div className="flex flex-wrap items-center justify-center sm:justify-start gap-3 text-sm text-gray-400">
            ...
            {jikanInfo?.status && (
              ...
              >
                {jikanInfo.status === 'Currently Airing'
                  ? 'En emisión'
                  : jikanInfo.status === 'Finished Airing'
                    ? 'Finalizado'
                    : jikanInfo.status}
              </span>
            )}
